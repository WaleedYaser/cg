...
constexpr uint32_t tri_count = 128;
	...
	uint32_t image_width, uint32_t image_height)
{
	...
	Vec3 p_screen = Vec3{p_cam.x / -p_cam.z, p_cam.y / -p_cam.z, 0.0f};
	Vec3 p_ndc = Vec3{
		(p_screen.x + canvas_width * 0.5f) / canvas_width,
		(p_screen.y + canvas_height * 0.5f) / canvas_height,
		0.0f};

	return Vec3 {p_ndc.x * image_width, (1.0f - p_ndc.y) * image_height, 0.0f};
}
...
main(int argc, char *argv[])
{
	...
	fprintf(file, "<svg version=\"1.1\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" xmlns=\"http://www.w3.org/2000/svg\" height=\"512\" width=\"512\">\n");

	Mat4 camera_to_world = mat4_new(
		0.871214f, 0.0f, -0.490904f, 0.0f,
	...
	float canvas_width = 2, canvas_height = 2;
	...
	for (uint32_t i = 0; i < tri_count; ++i)
	{
		...
		Vec3 v0_raster = compute_pixel_coord(
			v0_world, world_to_camera,
			canvas_width, canvas_height,
			image_width, image_height);

		Vec3 v1_raster = compute_pixel_coord(
			v1_world, world_to_camera,
			canvas_width, canvas_height,
			image_width, image_height);

		Vec3 v2_raster = compute_pixel_coord(
		...
		fprintf(
			...
			int(v0_raster.x), int(v0_raster.y), int(v1_raster.x), int(v1_raster.y));

		fprintf(
			file,
			"<line x1=\"%d\" y1=\"%d\" x2=\"%d\" y2=\"%d\" style=\"stroke:rgb(0,0,0);stroke-width:1\" />\n",
			int(v1_raster.x), int(v1_raster.y), int(v2_raster.x), int(v2_raster.y));

		fprintf(
			file,
			"<line x1=\"%d\" y1=\"%d\" x2=\"%d\" y2=\"%d\" style=\"stroke:rgb(0,0,0);stroke-width:1\" />\n",
			int(v2_raster.x), int(v2_raster.y), int(v0_raster.x), int(v0_raster.y));
	}
	fprintf(file, "</svg>\n");
